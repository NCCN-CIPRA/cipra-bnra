...
const riskFileData = {
  ...
  cr4de_scenario_considerable: "<table><tr><td>Considerable A</td></tr><tr><td>Considerable B</td></tr></table>",
...
describe("Standard Risk File Editor", () => {
        ...
        cy.bnraGet("cr4de_riskfileses", "$filter=cr4de_title eq 'Test Risk'")
          .then(([riskFile]) => {
            if (riskFile) {
              ...
              // Delete risk file
              return cy
                .bnraDelete("cr4de_riskfileses", token, riskFile.cr4de_riskfilesid)
                .then(() => cy.bnraCreate("cr4de_riskfileses", token, riskFileData));
            }
            return cy.bnraCreate("cr4de_riskfileses", token, riskFileData);
          })
          .then(() => {
            cy.get("button")
              .contains("Test Admin")
  ...
  describe.only("The hazard catalogue", () => {
  ...
  describe("Risk File Participants", () => {
          ...
          cy.wrap(cards[2])
            ...
            .then(() => {
              cy.wrap(cards[2])
                ...
                .then(() => {
                  cy.intercept("PATCH", "https://bnra.powerappsportals.com/_api/cr4de_riskfileses*").as(
                    "updateRiskFile"
                  );
    ...
    it("correctly shows scenarios", () => {
      ...
      cy.get(".MuiContainer-root > .MuiPaper-root")
        ...
        .then((cards) => {
          ...
          cy.wrap(cards[4]).should("contain.text", "Considerable A");
          cy.wrap(cards[4]).should("contain.text", "Considerable B");

          cy.wrap(cards[4]).should("contain.text", "Major A");
          cy.wrap(cards[4]).should("contain.text", "Major B");

          cy.wrap(cards[4]).should("contain.text", "Extreme A");
          cy.wrap(cards[4]).should("contain.text", "Extreme B");
        });
    });
