...
describe("Emerging Risk File Editor", () => {
  before(() => {
  ...
  it("allows editing of the horizon analysis", () => {
    cy.intercept("PATCH", "https://bnra.powerappsportals.com/_api/cr4de_riskfileses*").as("updateRiskFile");
    ...
    cy.get(".MuiContainer-root > .MuiPaper-root")
      ...
      .then((cards) => {
        return cy
          .wrap(cards[2])
          .should("contain.text", "2. Horizon Analysis")
          .get("[contenteditable]")
          .eq(0)
          .clear({ force: true })
          .type(randomInput, { delay: 100 });
  ...
  it("allows editing of catalysing effects", () => {
    ...
    cy.get(".MuiContainer-root > .MuiPaper-root")
      .should("have.length", 8)
      .then((cards) => {
        cy.wrap(cards[7]).should("contain.text", "3. Catalysing Effects").should("not.contain.text", "Definition of");
