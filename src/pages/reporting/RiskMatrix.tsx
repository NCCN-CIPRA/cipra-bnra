...
export default function RiskMatrix({ calculations }: { calculations: DVRiskFile<DVAnalysisRun>[] | null }) {
  ...
  const data = calculations
    .filter(
      (c) =>
        c.cr4de_latest_calculation &&
        c.cr4de_latest_calculation.cr4de_results &&
        (c.cr4de_latest_calculation.cr4de_results.tp !== 0 || c.cr4de_latest_calculation.cr4de_results.ti !== 0)
    )
    .map((c) => ({
      id: c.cr4de_riskfilesid,
      name: c.cr4de_title,
      category: c.cr4de_risk_category,
      x: c.cr4de_latest_calculation!.cr4de_results!.tp || 0.001,
      y: c.cr4de_latest_calculation!.cr4de_results!.ti || 1600000,
      z: 1,
    }));
