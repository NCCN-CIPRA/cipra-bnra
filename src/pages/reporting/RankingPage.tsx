...
import { CssBaseline, Container, Paper, Skeleton, Select, MenuItem, FormControl, Divider, Box } from "@mui/material";
  ...
  useEffect(() => {
    const getRiskFiles = async function () {
      try {
        ...
        ranking.sort((a: any, b: any) => (b.calculated[impactField] || 0) - (a.calculated[impactField] || 0));
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        ...
                        const sorted = riskFiles
                          ...
                          .sort(
                            (a: any, b: any) =>
                              (b.calculated[e.target.value] || 0) - (a.calculated[e.target.value] || 0)
                          );
              ...
              {riskFiles
                ? riskFiles.map((riskFile, i) => (
                    <TableRow
                      ...
                      onClick={() => navigate(`/reporting/${riskFile.cr4de_riskfilesid}`)}
                    >
                      <TableCell
                        component="th"
                        scope="row"
                      ...
                      <TableCell align="right">{Math.round(100 * riskFile.calculated[impactField]) / 100}</TableCell>
                    </TableRow>
                  ))
                : [1, 2, 3].map((i) => (
