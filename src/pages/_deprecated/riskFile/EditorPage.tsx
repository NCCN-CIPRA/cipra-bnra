...
export default function EditorPage() {
  ...
  const [validationProcessedDialogOpen, setValidationProcessedDialogOpen] =
    useState(false);
  const [
    validationNotProcessedDialogOpen,
    setValidationNotProcessedDialogOpen,
  ] = useState(false);
  ...
  const { data: participants, getData: getParticipants } = useLazyRecords<
  ...
  const { data: effects, getData: getEffects } = useLazyRecords<
    DVRiskCascade<undefined, SmallRisk>
  >({
    table: DataTable.RISK_CASCADE,
  });
  ...
  const { data: validations, getData: getValidations } = useLazyRecords<
    DVValidation<undefined, DVContact>
  >({
    table: DataTable.VALIDATION,
    saveOptions: true,
  });
  const { data: feedbacks, getData: getFeedback } = useLazyRecords<
    DVFeedback<DVContact>
  >({
    table: DataTable.FEEDBACK,
  });

  const { data: riskFile, reloadData: reloadRiskFile } =
    useRecord<ProcessedRiskFile>({
      table: DataTable.RISK_FILE,
  ...
  const unprocessedFeedback = (): {
    [key: string]: DVValidation<unknown, DVContact>[];
  } => {
    ...
    const filteredValidations = {
      Definition: validations.filter(
        (v) =>
          v.cr4de_definition_feedback !== null &&
          v.cr4de_definition_feedback_response === null
      ),
      "Historical Events": validations.filter(
        (v) =>
          v.cr4de_historical_events_feedback !== null &&
          v.cr4de_historical_events_feedback_response === null
      ),
      "Intensity Parameters": validations.filter(
        (v) =>
          v.cr4de_intensity_parameters_feedback !== null &&
          v.cr4de_intensity_parameters_feedback_response === null
      ),
      "Scenarios/Horizon Analysis": validations.filter(
        (v) =>
          v.cr4de_scenarios_feedback !== null &&
          v.cr4de_scenarios_feedback_response === null
      ),
      Causes: validations.filter(
        (v) =>
          v.cr4de_causes_feedback !== null &&
          v.cr4de_causes_feedback_response === null
      ),
      Effects: validations.filter(
        (v) =>
          v.cr4de_effects_feedback !== null &&
          v.cr4de_effects_feedback_response === null
      ),
      "Catalysing Effects": validations.filter(
        (v) =>
          v.cr4de_catalysing_effects_feedback !== null &&
          v.cr4de_catalysing_effects_feedback_response === null
      ),
    };

    return Object.entries(filteredValidations).reduce(
      (acc, [key, validations]) => {
  ...
  const handleSave = async () => {
    ...
    await Promise.all(
      feedbackRefs.current.map((validation) => {
        if (
          !validation.cr4de_bnravalidationid ||
  ...
  return (
    <>
      ...
      <Dialog
        open={validationProcessedDialogOpen}
        onClose={() => setValidationProcessedDialogOpen(false)}
      >
        <DialogTitle>
          Are you finished processing the validation feedback for this risk
          file?
        </DialogTitle>
        <DialogContent sx={{ pb: 0 }}>
          <DialogContentText>
            <Typography variant="body1" paragraph>
              You have the option to start a silent procedure or to organise a
              consensus meeting.
            </Typography>
          </DialogContentText>
          <DialogContentText>
            <Typography variant="body1" paragraph>
              If you choose a silent procedure, your experts will be
              automatically notified of this. After the end of the silent
              procedure, the validation step will be finalized automatically and
              the experts will no longer be able to change their input.{" "}
              <b>
                ATTENTION! If a silence procedure is already underway, Pressing
          ...
          <DialogContentText>
            <Typography variant="body1" paragraph>
              If you would like to organise a meeting, nothing will change and
              you will have to contact your experts manually. If you previously
              started a silence procedure, pressing this button will cancel it.
            </Typography>
          </DialogContentText>
        </DialogContent>
      ...
      <Dialog
        open={validationNotProcessedDialogOpen}
        onClose={() => setValidationNotProcessedDialogOpen(false)}
      >
        <DialogTitle>You have not responded to all feedback</DialogTitle>
        <DialogContent sx={{ pb: 0 }}>
          <DialogContentText>
            <Typography variant="body1" paragraph>
              You need to respond to each experts' feedback before you can start
              a silence procedure. Missing responses:
            </Typography>
          </DialogContentText>
          <DialogContentText>
            {Object.entries(unprocessedFeedback()).map(([field, missing]) => (
              <Typography variant="body1">
                Feedback on {field.toLowerCase()} by{" "}
                {missing.map((v) => v.cr4de_expert.emailaddress1).join(", ")}
              </Typography>
            ))}
          </DialogContentText>
        </DialogContent>
      ...
      >
        ...
        <Button
          ...
          onClick={async () => {
            ...
            if (
              participants &&
              participants.filter((p) => p.cr4de_validation_finished).length >=
                1
            ) {
