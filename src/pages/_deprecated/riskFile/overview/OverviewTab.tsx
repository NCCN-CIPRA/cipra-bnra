...
export default function OverviewTab({
  ...
  const startConsensus = async () => {
    ...
    await api.updateRiskFile(riskFile.cr4de_riskfilesid, {
      ...getConsensusRiskFile(
        getCompletedDirectAnalyses(riskFile, participants, directAnalyses)
      ),
    ...
    if (sendEmail) {
      if (consensus === CONSENSUS_TYPE.MEETING)
        await process.startConsensusMeeting(riskFile);
      else if (consensus === CONSENSUS_TYPE.SILENCE)
        await process.startConsensusSilenceProcedure(riskFile);
    }
  ...
  const checkConsensus = () => {
    const check = {
      weights: getConsensusRiskFile(
        getCompletedDirectAnalyses(riskFile, participants, directAnalyses)
      ),
      noWeights: getConsensusRiskFile(
        getCompletedDirectAnalyses(riskFile, participants, directAnalyses),
        false
      ),
      cascades: [] as any[],
  ...
  const fixConsensus = async () => {
    const weights = getConsensusRiskFile(
      getCompletedDirectAnalyses(riskFile, participants, directAnalyses)
    );
    ...
    await api.updateRiskFile(
      ...
      RISK_FILE_QUANTI_FIELDS.filter((f) => {
        if (
          riskFile.cr4de_discussion_required &&
          riskFile.cr4de_discussion_required[
            getParameter(f) as keyof DiscussionsRequired
          ] === DiscussionRequired.RESOLVED
        ) {
          return false;
        }
          ...
          <Box sx={{ height: 200 }}>
            {participants && (
              <ScoreCard
                ...
                calculation={
                  calculations[0] ? calculations[0].cr4de_results : null
                }
              />
            )}
          </Box>
        ...
        <Grid
          ...
          <Box sx={{ height: 200 }}>
            <HistoryCard
              riskFile={riskFile}
              calculations={
                calculations ? calculations.slice().reverse() : null
              }
            />
          </Box>
        </Grid>
              ...
              <CardContent>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      setConsensus(e.target.value as CONSENSUS_TYPE)
                    }
                  >
                    <MenuItem value={CONSENSUS_TYPE.SILENCE}>
                      Silence Procedure
                    </MenuItem>
                    <MenuItem value={CONSENSUS_TYPE.MEETING}>
                      Consensus Meeting
                    </MenuItem>
                    <MenuItem value={CONSENSUS_TYPE.NONE}>
                      No Consensus
                    </MenuItem>
                  </Select>
                </FormControl>
            ...
            <Card>
              <CardContent>
                {riskFile.cr4de_consensus_type === CONSENSUS_TYPE.MEETING &&
                  ...
                  new Date(riskFile.cr4de_consensus_date) >= new Date() && (
                    <Typography variant="body1">
                      Consensus meeting planned on{" "}
                      {format(
                        new Date(riskFile.cr4de_consensus_date),
                        "dd.MM.yyyy"
                      )}
                    </Typography>
                  )}
                {riskFile.cr4de_consensus_type === CONSENSUS_TYPE.MEETING &&
                  ...
                  new Date(riskFile.cr4de_consensus_date) < new Date() && (
                    <Typography variant="body1">
                      Consensus meeting took place on{" "}
                      {format(
                        new Date(riskFile.cr4de_consensus_date),
                        "dd.MM.yyyy"
                      )}
                ...
                {riskFile.cr4de_consensus_type === CONSENSUS_TYPE.NONE &&
                  riskFile.cr4de_consensus_date && (
                    <Typography variant="body1">
                      Consensus phase was skipped on{" "}
                      {format(
                        new Date(riskFile.cr4de_consensus_date),
                        "dd.MM.yyyy"
                      )}
              ...
              <CardActions sx={{ justifyContent: "flex-end" }}>
                <Tooltip
                  title={
                    riskFile.cr4de_consensus_date !== null &&
                    new Date(riskFile.cr4de_consensus_date) < new Date()
        ...
        <Grid sx={{ mt: 2 }} size={12}>
          <Card>
                ...
                <>
                  ...
                  <table>
                    ...
                    {test.cascades
                      .filter((c: any) => {
                        if (
                          c.cascade.cr4de_cause_hazard.cr4de_risk_type ===
                          RISK_TYPE.EMERGING
                        )
                          return false;
                        return CASCADE_ANALYSIS_QUANTI_FIELDS.some((f) => {
                          return (
                            ...
                            c.cascade.cr4de_discussion_required !==
                              DiscussionRequired.RESOLVED
                          );
                        });
                      })
                        ...
                        <>
                          <tr>
                            <td colSpan={4}>
                              {
                                (
                                  c.cascade as DVRiskCascade<
                                    SmallRisk,
                                    SmallRisk
                                  >
                                ).cr4de_cause_hazard.cr4de_title
                              }{" "}
                              causes{" "}
                              {
                                (
                                  c.cascade as DVRiskCascade<
                                    ...
                                    SmallRisk
                                  >
                                ).cr4de_effect_hazard.cr4de_title
                              }
                            </td>
                          </tr>
                          <tr>
                            {CASCADE_ANALYSIS_QUANTI_FIELDS.filter((f) => {
                              ...
                              <tr>
                                ...
                                {[c.cascade, c.noWeights, c.weights].map(
                                  (rf) => (
                                    <td
                                      style={{
                                        paddingLeft: 10,
            ...
            <CardActions sx={{ justifyContent: "flex-end" }}>
              ...
              <Button
                loading={isSaving}
                placeholder="Checking average values..."
