...
export default function ExpertGraph({
  ...
  const [firstDate] = useState<number>(new Date(2023, 1, 12).getTime());
  ...
  useEffect(() => {
    ...
    const firstDateCalc = firstDate;

    participations.forEach((p) => {
      if (!p._cr4de_contact_value) return;

      if (!experts[p._cr4de_contact_value])
        experts[p._cr4de_contact_value] = [];
    ...
    for (const e of Object.values(experts)) {
      for (let i = 0; i < dates.length; i++) {
        ...
        if (
          ...
          e.every(
            (p) =>
              p.cr4de_validation_finished_on &&
