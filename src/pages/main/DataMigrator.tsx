...
export default function DataMigrator() {
  ...
  const [cascadeDict, setCascadeDict] = useState<{ [key: string]: any } | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const getCascades = async () => {
      ...
      setCascades(
        responseJson.value.filter((c: any) => c.cr4de_risk_file === null)
      );
      setCascadeDict(
        responseJson.value.reduce(
          (acc: any, el: any) => ({
  ...
  useEffect(() => {
    if (
      ...
      cascadeAnalyses &&
      directAnalyses &&
      updatedDAs >= directAnalyses.length
    ) {
    ...
    <>
      <Container sx={{ mt: 4, pb: 8 }}>
        <Paper sx={{ p: 2 }}>
          <Typography>
            {riskFiles
              ? `Loaded ${Object.keys(riskFiles).length} risk files`
              : "Loading risk files..."}
          </Typography>
          <Typography>
            {cascades
              ? `Loaded ${cascades.length} cascades to migrate`
              : "Loading cascades..."}
          </Typography>
          {riskFiles && cascades && (
            <Typography sx={{ mb: 2 }}>
          ...
          {riskFiles && cascades && updatedCascades >= cascades.length && (
            <>
              ...
              {directAnalyses &&
                ...
                updatedDAs >= directAnalyses.length && (
                  <Typography sx={{ mb: 2 }}>
                    Updating cascade analyses ({updatedCAs}/
