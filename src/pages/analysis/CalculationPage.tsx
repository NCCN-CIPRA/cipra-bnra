...
const calculateTotalImpacts = (risk: any) => {
  ...
  risk.ti_Ha =
    risk.rp_c * risk.ti_Ha_c +
    risk.rp_m * risk.ti_Ha_m +
    risk.rp_e * risk.ti_Ha_e;
  risk.ti_Hb =
    risk.rp_c * risk.ti_Hb_c +
    risk.rp_m * risk.ti_Hb_m +
    risk.rp_e * risk.ti_Hb_e;
  risk.ti_Hc =
    risk.rp_c * risk.ti_Hc_c +
    risk.rp_m * risk.ti_Hc_m +
    risk.rp_e * risk.ti_Hc_e;
  risk.ti_Sa =
    risk.rp_c * risk.ti_Sa_c +
    risk.rp_m * risk.ti_Sa_m +
    risk.rp_e * risk.ti_Sa_e;
  risk.ti_Sb =
    risk.rp_c * risk.ti_Sb_c +
    risk.rp_m * risk.ti_Sb_m +
    risk.rp_e * risk.ti_Sb_e;
  risk.ti_Sc =
    risk.rp_c * risk.ti_Sc_c +
    risk.rp_m * risk.ti_Sc_m +
    risk.rp_e * risk.ti_Sc_e;
  risk.ti_Sd =
    risk.rp_c * risk.ti_Sd_c +
    risk.rp_m * risk.ti_Sd_m +
    risk.rp_e * risk.ti_Sd_e;
  risk.ti_Ea =
    risk.rp_c * risk.ti_Ea_c +
    risk.rp_m * risk.ti_Ea_m +
    risk.rp_e * risk.ti_Ea_e;
  risk.ti_Fa =
    risk.rp_c * risk.ti_Fa_c +
    risk.rp_m * risk.ti_Fa_m +
    risk.rp_e * risk.ti_Fa_e;
  risk.ti_Fb =
    risk.rp_c * risk.ti_Fb_c +
    risk.rp_m * risk.ti_Fb_m +
    risk.rp_e * risk.ti_Fb_e;
...
const calculateIndirectProbabilities = (risk: any) => {
  risk.causes.forEach((cause: any) => {
    cause.ip_c =
      cause.C2C * (cause.cause.tp_c || 0) +
      cause.M2C * (cause.cause.tp_m || 0) +
      cause.E2C * (cause.cause.tp_e || 0);
    cause.ip_m =
      cause.C2M * (cause.cause.tp_c || 0) +
      cause.M2M * (cause.cause.tp_m || 0) +
      cause.E2M * (cause.cause.tp_e || 0);
    cause.ip_e =
      cause.C2E * (cause.cause.tp_c || 0) +
      cause.M2E * (cause.cause.tp_m || 0) +
      cause.E2E * (cause.cause.tp_e || 0);

    cause.ip = cause.ip_c + cause.ip_m + cause.ip_e;
  });
  risk.ip_c = risk.causes.reduce(
    (acc: number, effect: any) => acc + effect.ip_c,
    0
  );
  risk.ip_m = risk.causes.reduce(
    (acc: number, effect: any) => acc + effect.ip_m,
    0
  );
  risk.ip_e = risk.causes.reduce(
    (acc: number, effect: any) => acc + effect.ip_e,
    0
  );
...
const calculateIndirectImpacts = (risk: any) => {
  ...
  risk.ii_Ha_c = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Ha_c,
    0
  );
  risk.ii_Hb_c = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Hb_c,
    0
  );
  risk.ii_Hc_c = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Hc_c,
    0
  );
  risk.ii_Sa_c = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sa_c,
    0
  );
  risk.ii_Sb_c = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sb_c,
    0
  );
  risk.ii_Sc_c = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sc_c,
    0
  );
  risk.ii_Sd_c = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sd_c,
    0
  );
  risk.ii_Ea_c = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Ea_c,
    0
  );
  risk.ii_Fa_c = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Fa_c,
    0
  );
  risk.ii_Fb_c = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Fb_c,
    0
  );

  risk.ii_Ha_m = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Ha_m,
    0
  );
  risk.ii_Hb_m = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Hb_m,
    0
  );
  risk.ii_Hc_m = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Hc_m,
    0
  );
  risk.ii_Sa_m = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sa_m,
    0
  );
  risk.ii_Sb_m = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sb_m,
    0
  );
  risk.ii_Sc_m = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sc_m,
    0
  );
  risk.ii_Sd_m = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sd_m,
    0
  );
  risk.ii_Ea_m = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Ea_m,
    0
  );
  risk.ii_Fa_m = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Fa_m,
    0
  );
  risk.ii_Fb_m = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Fb_m,
    0
  );

  risk.ii_Ha_e = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Ha_e,
    0
  );
  risk.ii_Hb_e = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Hb_e,
    0
  );
  risk.ii_Hc_e = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Hc_e,
    0
  );
  risk.ii_Sa_e = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sa_e,
    0
  );
  risk.ii_Sb_e = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sb_e,
    0
  );
  risk.ii_Sc_e = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sc_e,
    0
  );
  risk.ii_Sd_e = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Sd_e,
    0
  );
  risk.ii_Ea_e = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Ea_e,
    0
  );
  risk.ii_Fa_e = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Fa_e,
    0
  );
  risk.ii_Fb_e = risk.effects.reduce(
    (acc: number, effect: any) => acc + effect.ii_Fb_e,
    0
  );
...
export default function CalculationPage() {
  ...
  const calculateProbabilities = async (rfs: any[], innerLog: string[]) => {
    ...
    for (let run = 1; run <= 10; run++) {
      ...
      const deltaTotalProbability = Math.abs(
        totalProbability - lastTotalProbability
      );

      if (deltaTotalProbability < 0.001) {
        innerLog.push(
          `Convergence reached (Total Probability: ${
        ...
        innerLog.push(
          `No convergence (Total Probability: ${
            Math.round(1000 * totalProbability) / 1000
          }, Delta: ${Math.round(1000 * deltaTotalProbability) / 1000})...`
        );
        setLog(innerLog);
  ...
  const calculateImpacts = async (rfs: any[], innerLog: string[]) => {
    ...
    for (let run = 1; run <= 10; run++) {
      ...
      if (deltaTotalImpact < 1) {
        innerLog.push(
          `Convergence reached (Total Impact: ${Math.round(
            totalImpact
          )}, Delta: ${Math.round(deltaTotalImpact)})...`
        );
        setLog(innerLog);
        break;
      } else {
        innerLog.push(
          `No convergence (Total Impact: ${Math.round(
            totalImpact
          )}, Delta: ${Math.round(deltaTotalImpact)})...`
        );
        setLog(innerLog);
  ...
  const saveResults = async () => {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      await fetch(
        `https://bnra.powerappsportals.com/_api/cr4de_riskfileses(${rf.cr4de_riskfilesid})`,
        {
          method: "PATCH",
          headers: {
            Authorization:
          ...
          body: JSON.stringify({
            cr4de_calculated: JSON.stringify({
              dp_c: rf.dp_c,
              dp_m: rf.dp_m,
              dp_e: rf.dp_e,
              ...
              causes: rf.causes.map((c: any) => ({
                risk: c.cause.cr4de_riskfilesid,
                ip_c: c.ip_c,
                ip_m: c.ip_m,
                ip_e: c.ip_e,
      ...
      innerLog.splice(
        innerLog.length - 1,
        1,
        `Saving calculations (${i + 1}/${results.length})`
      );
      setLog(innerLog);
    }
  ...
  return (
    <>
      <Container sx={{ mt: 4, pb: 8 }}>
        <Paper sx={{ p: 2 }}>
          {(!riskFiles || !cascades) && (
            <Typography variant="body1">Loading Risk Files...</Typography>
          )}
